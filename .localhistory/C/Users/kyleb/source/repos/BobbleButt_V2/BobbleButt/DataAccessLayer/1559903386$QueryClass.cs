...
namespace BobbleButt.DataAccessLayer
{
    public class QueryClass
    {
        ...
        public static void AddOrder(Order o)
        {
            using (SqlConnection connection = new SqlConnection(m_connectionString))
            {
                string sql = "BEGIN TRAN INSERT INTO  [Order] (email, postageOptionsID, status, streetAddress, suburb, postcode, phone, date, total) VALUES " +
                    "(@email, @postageid, @status, @street, @suburb, @postcode, @phone, @date, @total)";
                using (SqlCommand command = new SqlCommand(sql, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@street", o.Total);
                    connection.Open();
                    SqlDataReader reader = command.ExecuteReader();
                    reader.Close();
                }
                foreach (Product p in o.Products)
                {
                    ...
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        command.Parameters.AddWithValue("@product", p.ID);
